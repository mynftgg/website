...
- Add your NFT images and metadata to `./assets/` following the hackmd guide.
- Do not create your `.env` file until completing this section!
...
solana airdrop 10
metaplex upload ./assets --env devnet --keypair ~/.config/solana/devnet.json 0 -n 2 # n is the number of assets, the 0 means start from idx 0. This creates the .cache/devnet-temp entry which contains the value for REACT_APP_CANDY_MACHINE_CONFIG.
metaplex verify -e devnet -k ~/.config/solana/devnet.json
metaplex create_candy_machine -e devnet -k ~/.config/solana/devnet.json -p 1 # This returns the value for REACT_APP_CANDY_MACHINE_ID.
metaplex set_start_date -e devnet -k ~/.config/solana/devnet.json "04 Dec 2022 00:12:00 GMT" # This returns the value for REACT_APP_CANDY_START_DATE.
```

- Use the returned values from the above commands to fill out `.env`.
